...
class YahtzeeBoard:
    ...
    def rollDiceListener(self): #rollDiceListener
        for i in range(5):
            if (self.diceButtons[i]['state']!='disabled'):
                ...
                self.diceButtons[i].configure(text=str(self.dice[i]))
    ...
    def categoryListener(self,row):
        ...
        if (self.round == 13):
            if self.players[0].getTotalScore() > self.players[1].getTotalScore():
                tkinter.messagebox.showinfo("result","1번 플레이어 승")
            else:
                tkinter.messagebox.showinfo("result","2번 플레이어 승")
